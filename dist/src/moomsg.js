...
var MooMessage = /** @class */ (function () {
    ...
    MooMessage.prototype.send_continue = function () {
        ...
        var header = 'MOO/1 CONTINUE ' + name + '\n' +
        ...
        if (this.msg.log)
            this.moo.logger.log('-> CONTINUE', this.msg.request_id, name, origbody ? JSON.stringify(origbody) : "");
    ...
    MooMessage.prototype.send_complete = function () {
        ...
        if (this.msg.log)
            this.moo.logger.log('-> COMPLETE', this.msg.request_id, name, origbody ? JSON.stringify(origbody) : "");
        var m = Buffer.from(header + '\n');
        if (body)
            this.moo.transport.send(Buffer.concat([m, body], m.length + body.length));
        else
