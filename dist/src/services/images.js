...
RoonApiImage.prototype.get_image = function () {
    var i = 0;
    var image_key = arguments[i++];
    var opts = {};
    if (typeof (arguments[i]) != "function")
        opts = arguments[i++];
    var cb = arguments[i++];
    opts = Object.assign({ image_key: image_key }, opts);
    this.core.moo.send_request(SVCNAME + "/get_image", opts, function (msg, body) {
        if (msg && msg.name == "Success")
            cb(false, msg.content_type, body);
        else
            cb(msg ? msg.name : "NetworkError");
    });
};
